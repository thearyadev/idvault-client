...
>((props, ref: ForwardedRef<BottomSheetMethods>) => {
  const docData =
    DocTypes[props.document?.documentType as keyof typeof DocTypes] || null; // can be null if document is undefined.
  // if document is undefined, then docData will be null.
  // ? must be used for safe access.
  return (
    <BottomSheet
      ref={ref}
      style={{ ...bottomSheetStyles.bottomSheet, padding: 0 }}
      hideDragHandle
      height="90%"
    >
      ...
      <View
        style={{
          margin: 10,
          display: props.confirmationCallback ? "flex" : "none",
        }}
      >
        <Pressable
          style={buttonStyle.buttonStyle}
          onPress={props.confirmationCallback}
        >
...
export function BirthCertificateSheet({
  ...
  return (
    ...
    >
      ...
      <Cell
        itemKey="date of registration"
        value={document?.dateOfRegistrtion}
      />
